                          ...
                          longlong *func_value)
{
  ...
  else // row is historical
  {
    ...
    uint max_hist_id= vers_info->hist_part->id;
    uint min_hist_id= 0, loc_hist_id;
    ulong unused;
    my_time_t ts;
    ...
    if ((max_hist_id == 0 || range_value[max_hist_id - 1] < ts) &&
        range_value[max_hist_id] >= ts)
      goto done; // fastpath

    max_hist_id= part_info->num_parts - 1;
                            ...
                            alter_info->partition_names))
        {
          if (tab_part_info->part_type == VERSIONING_PARTITION)
          {
            ...
            if (tab_part_info->vers_info->interval.is_set())
            {
              ...
              tab_part_info->vers_info->interval.start= part_elem->range_value;
            }
          }
          /*
